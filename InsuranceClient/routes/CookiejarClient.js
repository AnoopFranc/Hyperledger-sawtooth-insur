...
get_address(publicKey){
   var Address= hash(FAMILY_NAME).substr(0, 6) + "00" + hash(publicKey).substr(0, 62);
   return Address
}

 async send_data(action,values, pData={}){
  var payload = ''
...
async getOrderListings() {
  let orderListingAddress = hash(FAMILY_NAME).substr(0, 6) + '01';
